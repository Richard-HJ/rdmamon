#!/usr/bin/perl
# cmd_jitter_ud.pl open date-time file, 
#                     use rdmamon_ud_mon to measure packet jitter as function of packet spacing & size
#input 
#   -a= CPU affinity
...
my $rdmamon_prog = "rdmamon_ud_mon";
my $rdmamon_path;
...
for($pkt_index=0; $pkt_index<=$pkt_index_max; $pkt_index++){
    ...
    $cmd = $rdmamon_path.$cmd_rdmamon." -d ".$opt_d." -I ".$opt_I." -w ".$wait." -M ".$bin_min." -B ".$bin." -l ".$opt_l;   
    $out =`$cmd 2>&1`;
...
for($pkt_index=0; $pkt_index<=$pkt_index_max; $pkt_index++){
    ...
    # make the rdmamon command
    $cmd = $rdmamon_path.$cmd_rdmamon_100." -d ".$opt_d." -I ".$opt_I." -w ".$wait." -M ".$bin_min." -B ".$bin." -l ".$opt_l;   
    $out =`$cmd 2>&1`;
    chomp($out);  # remove terminating newline
    print LOG $out."\n";
