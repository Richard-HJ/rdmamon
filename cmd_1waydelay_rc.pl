#!/usr/bin/perl
# cmd_1waydelay_rc.pl open date-time file, 
#                     use udpmon_rc_mon to measure 1-way delay as function of packet spacing
#input 
#   -a= CPU affinity
#   -d= remote host 
#   -l= no packets
...
for($pkt_index=0; $pkt_index<=$pkt_index_max; $pkt_index++){
    ...
    $cmd = $rdmamon_path.$cmd_rdmamon." -d ".$opt_d." -p ".$opt_p." -w ".$wait." -l ".$opt_l." -G ".$opt_l;   
    $out =`$cmd 2>&1`;
